...
**标准Nginx用户**（推荐）：
- ✅ **脚本会自动安装配置文件**到 `/etc/nginx/sites-available/`
- ✅ **自动创建软链接**到 `sites-enabled/`
- ✅ **自动测试配置**并询问是否重载Nginx
- ✅ **支持Certbot自动申请SSL证书**（可选）
- ✅ **配置完成后即可使用，无需手动操作**

**宝塔面板用户**：
- ⚠️ 由于宝塔面板的限制，脚本会生成配置文件并保存到项目目录
- 📋 配置文件路径：`项目目录/${DOMAIN}.conf`
- 📝 需要手动将配置文件内容复制到宝塔面板：
  1. 登录宝塔面板
  2. 网站 → 添加站点（如果未创建）
  3. 网站 → 您的域名 → 设置 → 配置文件
...
**重要提示**：
- ⚠️ 请立即登录并修改默认密码
- ✅ **域名和Nginx已通过选项2一键配置完成**（如果已使用选项2）
- ✅ **标准Nginx环境：配置已自动安装，无需手动操作**
- ✅ **宝塔面板：需要将生成的配置文件复制到宝塔面板（见上方说明）**
- ✅ 所有服务管理操作可通过菜单完成，无需记忆命令

**✅ 配置完成后的验证**：
- 使用 **选项7：查看服务状态** 检查服务是否正常运行
- 使用 **选项8：查看服务日志** 查看是否有错误
- 访问您的域名，检查网站是否正常显示
...
**⚠️ 注意**：如果您使用了**方式一（安装脚本）**，则**不需要**执行以下手动安装步骤。安装脚本已经自动完成了所有配置。
...
如果您已经使用安装脚本完成了安装，请直接跳转到[支付配置](#支付配置)章节。
...
**⚠️ 重要提示**：如果您使用了安装脚本的**选项2（配置域名和Nginx）**，则**不需要**执行以下步骤，配置已经自动完成。
