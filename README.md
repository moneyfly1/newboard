...
一键安装脚本提供了完整的交互式菜单，可以完成系统安装、域名配置、Nginx配置、服务管理、管理员管理等所有操作，**无需手动配置**。
...
#### 步骤2：配置域名和Nginx（选项2）⭐ **一键配置**

选择 **选项2：配置域名和Nginx**，这是**一键配置功能**，无需手动操作：

1. **输入域名**：例如 `example.com`（无需输入协议）
2. **选择是否使用HTTPS**：默认使用HTTPS（推荐）
...
**标准Nginx用户**：
- 脚本会自动安装配置文件到 `/etc/nginx/sites-available/`
- 自动创建软链接到 `sites-enabled/`
- 自动测试配置并询问是否重载Nginx
- 支持Certbot自动申请SSL证书
...
- **选项7：查看服务状态** - 检查服务运行状态、进程信息、端口监听等
- **选项8：查看服务日志** - 查看实时日志或最近日志
...
- **选项1：安装系统**
  - 完整安装流程，包括环境检查、依赖安装、数据库初始化、前端构建等
  - 自动安装Redis缓存服务（如果未安装）
  - 创建systemd服务并启动
...
- **选项4：启动服务** - 启动CBoard后端服务
- **选项5：停止服务** - 停止CBoard后端服务
- **选项6：重启服务** - 重启CBoard后端服务
- **选项7：查看服务状态** - 显示服务运行状态、进程信息、端口监听、最近日志
- **选项8：查看服务日志** - 支持实时日志、最近N行、错误日志等多种查看方式

#### 【管理员管理】

- **选项9：重设管理员密码** - 重置管理员账户密码
- **选项10：查看管理员账号** - 查看所有管理员账户信息
...
**重要提示**：
- ⚠️ 请立即登录并修改默认密码
- ✅ **域名和Nginx已通过选项2一键配置完成**（如果已使用）
- ✅ 所有服务管理操作可通过菜单完成，无需记忆命令
...
1. **创建网站**
   ...
   - 根目录：`/www/wwwroot/YOUR_DOMAIN/frontend/dist`（请将 `YOUR_DOMAIN` 替换为您的实际域名）
   - PHP版本：**纯静态**
   - 点击 **确定**
